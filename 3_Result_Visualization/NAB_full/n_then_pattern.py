...
def analyze_folder(folder_path, folder_name, output_base, ground_truth_timestamps):
    ...
    orig_path = os.path.join(folder_path, "")
    ...
    try:
        ...
        # Convert to Datetime Objects (The 'Orig' Timestamps)
        ...
        try:
            ...
            if not merged_successfully:
                if len(det_df) != len(df):
                    print(f"      [Warning] {det_name} length mismatch ({len(det_df)} vs {len(df)}). Forcing Index Merge.")
    ...
    if 'orig_value' in df.columns:
        ...
        if len(det_idx) > 0:
            ax.scatter(det_idx, df.loc[det_idx, 'orig_value'], c='black', marker='x', s=50, label='Detector('+ str(df['is_detector_anomaly'].sum()) + ")", zorder=5)

        # Plot Pattern Alerts
        pat_idx = df.index[df['is_pattern_anomaly'] == 1]
        if len(pat_idx) > 0:
            ax.scatter(pat_idx, df.loc[pat_idx, 'orig_value'], c='red', marker='o', s=20, label='Pattern Break('+ str(df['is_pattern_anomaly'].sum()) + ")", zorder=10)

        # Plot Ground Truth
        if gt_count > 0:
            gt_idx = df.index[df['is_ground_truth'] == 1]
            ax.scatter(gt_idx, df.loc[gt_idx, 'orig_value'], c='green', marker='*', s=200, label='Ground Truth(' + str(gt_count) + ")", zorder=4)
            for i in gt_idx:
        ...
        # We use the indices for plotting continuity, but label them with the Timestamp string
        step = max(1, len(df) // 12)
        ax.set_xticks(indices[::step])
        
        # Format: YYYY-MM-DD (Removing hour as requested earlier, but ensuring it comes from the 'Orig' file)
