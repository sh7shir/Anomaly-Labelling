...
ROLLING_WINDOW = 288  
PATTERN_STD_MULT = 3.5
EVALUATION_WINDOW = 100  # +/- 50 steps (approx 4 hours) window for matching
...
def get_continuous_events(binary_series):
    """
    Converts a binary pandas series (0/1) into a list of (start_index, end_index) tuples.
    """
    if binary_series.sum() == 0:
        return []
    
    # Find runs of 1s
    # Pad with 0 to detect start/end correctly even if at edges
...
def calculate_window_metrics(df, pred_col, gt_timestamps, window_size=EVALUATION_WINDOW):
    ...
    # --- RECALL: Fraction of GT anomalies detected ---
    tp_gt = 0
    used_pred_events = set() # Track which predictions were valid

    for gt_idx in gt_indices:
        start_w = max(0, gt_idx - window_size)
        end_w = min(len(df) - 1, gt_idx + window_size)
        
        # Check overlap with ANY prediction event
        found = False
    ...
    # --- PRECISION: Fraction of Predicted Events that matched a GT ---
...
def analyze_folder(folder_path, folder_name, output_base, ground_truth_timestamps):
    ...
    # C. COMBINED ANALYSIS (Union)
    # This combines the results of Detectors AND Pattern into one final result
    ...
    print(f"   -> Found {df['is_final_anomaly'].sum()} Total Combined Anomalies (Points)")
    ...
    if gt_count > 0:
        ...
        # Calculate metrics for the COMBINED result
    ...
    if 'orig_value' in df.columns:
        ...
        ax.set_title(f'Combined Analysis: {folder_name}\n(Detectors + Pattern)')
        ...
        # Plot COMBINED Alerts
        # We plot distinct markers for pure detector, pure pattern, and overlapping
        
        # 1. Detectors
        det_idx = df.index[df['is_detector_anomaly'] == 1]
        if len(det_idx) > 0:
            ax.scatter(det_idx, df.loc[det_idx, 'orig_value'], c='black', marker='x', s=50, label='Detector Alert', zorder=8)

        # 2. Pattern
        pat_idx = df.index[df['is_pattern_anomaly'] == 1]
        if len(pat_idx) > 0:
            ax.scatter(pat_idx, df.loc[pat_idx, 'orig_value'], c='red', marker='o', s=20, label='Pattern Alert', zorder=9)
        ...
        if gt_count > 0:
            ...
            # Visualize the Evaluation Window
            for i in gt_idx:
