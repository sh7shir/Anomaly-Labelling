...
# Now RETURNS metrics dict for global "overall score".
...
def analyze_folder(folder_path, folder_name, output_base, data_online_dir):
    ...
    plt.rcParams['axes.labelsize'] = 20       # X/Y label font size
    ...
    if gt_count > 0:
        ...
        # Draw AW spans + BORDER for anomaly windows
        if aws > 0:
            for i in gt_idx:
                start_w = max(0, i - aws)
                ...
                # Light fill
                ax.axvspan(start_w, end_w, color='green', alpha=0.08, zorder=1)

                # Border (no fill, dashed line)
                ax.axvspan(
                    start_w, end_w,
                    facecolor='none',
                    edgecolor='green',
    ...
    # This frees up more horizontal space for the time series.
    legend = ax.legend(
        loc='upper left',
    ...
    # Make layout nice with external legend
    plt.tight_layout()
    plt.savefig(os.path.join(out_plot, f"plot_{folder_name}.png"),
                bbox_inches="tight")  # ensures legend is not cut off
    plt.show()
    plt.close()


    # Return metrics for global aggregation
    return {
        "folder": folder_name,
        "tp": tp,
        "fp": fp,
        ...
        "aws": aws,
        "num_points": len(df),
        "num_final_anoms": total_combined
    }
...
if __name__ == "__main__":
    ...
    with os.scandir(base_dir) as entries:
        for entry in entries:
            ...
            res = analyze_folder(folder_path, entry.name, results_dir, data_online_dir)
            if res is not None:
    ...
    if found_any:
        ...
        # --- GLOBAL OVERALL SCORE ACROSS ALL FOLDERS ---
        df_summary = pd.DataFrame(all_results)
        summary_path = os.path.join(results_dir, "summary_metrics.csv")
        df_summary.to_csv(summary_path, index=False)
        print(f"Per-folder metrics saved to: {summary_path}")

        # Use only folders that actually have ground truth
        valid = df_summary[df_summary["gt_count"] > 0]

        if not valid.empty:
            global_tp = valid["tp"].sum()
            global_fp = valid["fp"].sum()
            global_tn = valid["tn"].sum()
            global_fn = valid["fn"].sum()

            g_prec, g_rec, g_f1, g_mcc = calculate_metrics(
                global_tp, global_fp, global_tn, global_fn
            )

            print("\n" + "=" * 60)
            print("GLOBAL OVERALL SCORE (aggregated over all folders with GT)")
            print(f"  Global TP:      {global_tp:.3f}")
            print(f"  Global FP:      {global_fp:.3f}")
            print(f"  Global TN:      {global_tn:.3f}")
            print(f"  Global FN:      {global_fn:.3f}")
            print("-" * 40)
            print(f"  Precision:      {g_prec:.4f}")
            print(f"  Recall:         {g_rec:.4f}")
            print(f"  F1-Score:       {g_f1:.4f}")
            print(f"  MCC (adj):      {g_mcc:.4f}")
            print("=" * 60 + "\n")
        else:
            print("\n[Info] No folders with ground truth. Global score not computed.")
    else:
