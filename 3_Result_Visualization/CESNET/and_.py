...
def analyze_metric_folder(folder_path, dataset_name, metric_name, output_base, ground_truth_timestamps, global_time_df, holiday_map):
    ...
    # C. Combine Anomalies (STRICT AND LOGIC)
    # Only flag if BOTH Detector AND Pattern agree
    ...
    print(f"   -> Alerts: Total (AND Logic)={df['is_anomaly'].sum()} | GT Matches={gt_count}")
    ...
    if 'final_value' in df.columns:
        ...
        ax.set_title(f'Analysis: {dataset_name} - {metric_name}\n(Requirement: Detectors AND Pattern Break)')
        
        indices = df.index
        
        # 1. SHADE WEEKENDS & HOLIDAYS
        ...
        # 2. SHADE ANOMALY WINDOWS (Grey)
        # Based on STRICT AND LOGIC
        ...
        if anomaly_indices:
            ...
            for s, e in zip(starts, ends):
                ax.axvspan(s, e, color='grey', alpha=0.4, zorder=1, label='Strict Anomaly' if s == starts[0] else "")
        ...
        # 4. ALERTS & GT POINTS
        # Plot individual contributors for debugging context
        ...
        if anomaly_indices:
            handles.append(Patch(facecolor='grey', alpha=0.4, label='Strict Anomaly (Both)'))
        ...
        config_text = (
            f"Config:\n"
            f"Mode: STRICT (Detector AND Pattern)\n"
