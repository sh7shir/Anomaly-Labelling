...
ROLLING_WINDOW = 24  
PATTERN_STD_MULT = 5
ANOMALY_WINDOW_SIZE = 100  # Size of the anomaly window to visualize
...
def analyze_metric_folder(folder_path, dataset_name, metric_name, output_base, ground_truth_timestamps, global_time_df, holiday_map):
    ...
    # --- Window Logic ---
    df['raw_anomaly'] = ((df['is_detector_anomaly'] == 1) | (df['is_pattern_anomaly'] == 1)).astype(int)
    # Create the window mask (centered)
    df['anomaly_window_mask'] = df['raw_anomaly'].rolling(window=ANOMALY_WINDOW_SIZE, center=True, min_periods=1).max().fillna(0).astype(int)
    ...
    print(f"   -> Alerts: Detector={df['is_detector_anomaly'].sum()} | Pattern={df['is_pattern_anomaly'].sum()} | GT Matches={gt_count}")
    ...
    if 'orig_value' in df.columns:
        ...
        for d in unique_dates:
            ...
            if d_type in ['Weekend', 'Holiday']:
                ...
                if len(d_indices) > 0:
                    ...
                    ax.axvspan(start_idx, end_idx, color=color, alpha=0.3, zorder=0, lw=0)

        # 2. SHADE ANOMALY WINDOWS (Background Layer 5)
        diffs = np.diff(df['anomaly_window_mask'].values, prepend=0, append=0)
        ...
        if len(starts) > 0:
            has_window = True
            ax.axvspan(starts[0], ends[0], color='lightgrey', alpha=0.8, zorder=5, label='Anomaly Window')
            for i in range(1, len(starts)):
                 ax.axvspan(starts[i], ends[i], color='lightgrey', alpha=0.8, zorder=5)

        # 3. PLOT DATA (Layer 10)
        ax.plot(indices, df['orig_value'], color='#1f77b4', alpha=0.8, label='Value', zorder=10)
        ax.plot(indices, df['expected_pattern'], color='black', linestyle='--', alpha=0.5, label='Trend', zorder=10)

        # 4. PLOT ANOMALY POINTS (Layer 15)
        # Detector Votes (Black X)
        det_idx = df.index[df['is_detector_anomaly'] == 1]
        if len(det_idx) > 0:
            ax.scatter(det_idx, df.loc[det_idx, 'orig_value'], c='black', marker='x', s=50, label=f'Detector Alert', zorder=15)

        # Pattern Breaks (Red Circle)
        pat_idx = df.index[df['is_pattern_anomaly'] == 1]
        if len(pat_idx) > 0:
            ax.scatter(pat_idx, df.loc[pat_idx, 'orig_value'], c='red', marker='o', s=20, label=f'Pattern Break', zorder=16)

        # 5. GROUND TRUTH (Layer 20)
        if gt_count > 0:
            gt_idx = df.index[df['is_ground_truth'] == 1]
            ax.scatter(gt_idx, df.loc[gt_idx, 'orig_value'], c='green', marker='*', s=200, label=f'Ground Truth', zorder=20)
            for i in gt_idx:
                ax.axvline(x=i, color='green', alpha=0.3, zorder=19)

        # 6. LEGEND & CONFIG
        handles, labels = ax.get_legend_handles_labels()
        
        if has_weekend:
            handles.append(Patch(facecolor='lightgreen', alpha=0.3, label='Weekend'))
        if has_holiday:
            handles.append(Patch(facecolor='lightpink', alpha=0.3, label='Holiday'))
        ...
        config_text = (
            f"Config:\n"
            f"Score Thresh: {DETECTOR_SCORE_THRESH}\n"
            f"Min Votes: {MIN_VOTES_FOR_ALARM}\n"
            f"Window: {ROLLING_WINDOW}\n"
            f"Anom Window: {ANOMALY_WINDOW_SIZE}"
        )
        ax.text(0.01, 0.99, config_text, transform=ax.transAxes, fontsize=9,
                verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor='gray'), zorder=30)
