...
This file allows exporting data to .csv files.
"""
...
def init_to_csv(self, export_filename):
    if not self.EVAL_EXPORT:
        # creating file
        current_time = datetime.now().strftime('%Y%m%d%H%M%S')
        result_filename = 'AnDePeD/results/{}{}_results.csv'.format(current_time, self.inst_num)
        param_filename = 'AnDePeD/results/{}{}_hyperparams.csv'.format(current_time, self.inst_num)
        ...
        result_file_path = export_filename
    ...
    if not self.EVAL_EXPORT:
        with open(param_filename, 'w', newline='') as csv_file__:
            ...
            row = {
                'hyperparameter': 'B',
                'value': self.B,
                'description': 'Look-back parameter.'
            }
            csv_writer__.writerow(row)
            row = {
                'hyperparameter': 'F',
                'value': self.F,
                'description': 'Predict-forward parameter.'
            }
            csv_writer__.writerow(row)
            row = {
