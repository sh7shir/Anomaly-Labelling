...
from NAB.nab.runner import Runner
from NAB.nab.util import (detectorNameToClass, checkInputs)
...
if __name__ == "__main__":
  ...
  if "bayesChangePt" in args.detectors:
    from NAB.nab.detectors.bayes_changept.bayes_changept_detector import (
      BayesChangePtDetector)
  if "null" in args.detectors:
    from NAB.nab.detectors.null.null_detector import NullDetector
  if "random" in args.detectors:
    from NAB.nab.detectors.random.random_detector import RandomDetector
  ...
  if "skyline" in args.detectors:
    from NAB.nab.detectors.skyline.skyline_detector import SkylineDetector
  if "windowedGaussian" in args.detectors:
    from NAB.nab.detectors.gaussian.windowedGaussian_detector import (
      WindowedGaussianDetector)
  if "knncad" in args.detectors:
    from NAB.nab.detectors.knncad.knncad_detector import KnncadDetector
  if "relativeEntropy" in args.detectors:
    from NAB.nab.detectors.relative_entropy.relative_entropy_detector import (
      RelativeEntropyDetector)
  if "expose" in args.detectors:
    from NAB.nab.detectors.expose.expose_detector import ExposeDetector
  if "contextOSE" in args.detectors:
    from NAB.nab.detectors.context_ose.context_ose_detector import (
    ContextOSEDetector )
  if "earthgeckoSkyline" in args.detectors:
    from NAB.nab.detectors.earthgecko_skyline.earthgecko_skyline_detector import EarthgeckoSkylineDetector
  if "ARTime" in args.detectors:
    from NAB.nab.detectors.ARTime.ARTime_detector import ARTimeDetector
